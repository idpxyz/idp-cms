...
interface Props {
  ...
  lang?: string;
  channel?: string;
  useTestApi?: boolean; // 新增：是否使用测试API
}

export default function TestHeadlinesModule({ 
  count = 2, 
  countSm, 
  countMd, 
  countLg = 4, 
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        setError(null);
        console.log(`🔄 开始加载头条数据，使用${useTestApi ? '测试' : '生产'}API...`);

        let apiUrl: string;
        if (useTestApi) {
          // 使用测试API，添加随机种子实现换一换
          apiUrl = `/api/test/headlines/?size=${effectiveCount}&seed=${refreshKey}`;
        } else if (channel) {
          // 特定频道的头条，添加偏移量实现换一换
          ...
          apiUrl = `/api/agg/headlines/?size=${batchSize}&site=localhost&diversity=high`;
        }
        ...
        let itemsData = useTestApi ? data.headlines : (data.headlines || data.items || []);
        
        // 对于生产API，实现前端循环切片
        if (!useTestApi && !channel && itemsData.length > 0) {
    ...
    load();
  }, [effectiveCount, channel, region, lang, useTestApi, refreshKey]); // 添加refreshKey支持换一换

  const titlePrefix = useTestApi ? "测试头条" : "今日头条";
    ...
    <div className="p-0">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-bold text-gray-900 flex items-center">
          ...
          {titlePrefix}
          {useTestApi && <span className="ml-2 text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">测试模式</span>}
        </h3>
        <button 
          onClick={() => {
