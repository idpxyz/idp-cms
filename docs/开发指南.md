...
本项目基于IDP-CMS架构，构建了一个专业的AI门户网站（aivoya.com），提供AI工具导航、行业资讯聚合和技术教程等功能。项目采用现代化的技术栈，包括Wagtail CMS作为内容管理后端，Next.js作为前端框架。
...
- **AI工具导航**: 分类展示各类AI工具，支持搜索和筛选
- **AI资讯聚合**: 实时聚合AI行业动态和技术突破
- **技术教程**: 提供AI相关的技术教程和学习资源
- **内容管理**: 基于Wagtail的强大内容管理系统
- **搜索功能**: 基于OpenSearch的全文搜索和推荐系统
...
class AIToolPage(Page):
    ...
    category = models.CharField(
        max_length=50,
        choices=[
            ('text-generation', '文字生成'), ('image-generation', '图像生成'),
            ('video-generation', '视频生成'), ('code-generation', '代码生成'),
            ('audio-generation', '音频生成'), ('data-analysis', '数据分析'),
            ('chatbot', '聊天机器人'), ('translation', '翻译'),
            ('productivity', '生产力'), ('research', '研究'), ('other', '其他'),
        ],
        default='other',
        verbose_name="工具分类"
    )
    pricing = models.CharField(
        max_length=20,
        choices=[('free', '免费'), ('freemium', '免费版'), ('paid', '付费'), ('enterprise', '企业版')],
        default='free',
        verbose_name="定价模式"
    )
    ...
    content_panels = Page.content_panels + [
        FieldPanel('description'), FieldPanel('tool_url'), FieldPanel('logo_url'),
        FieldPanel('category'), FieldPanel('pricing'), FieldPanel('features'),
        FieldPanel('rating'), FieldPanel('usage_count'), FieldPanel('is_hot'),
        FieldPanel('is_new'), FieldPanel('tags'), FieldPanel('channel_slug'),
        FieldPanel('region'), FieldPanel('language'),
    ]
...
class AINewsPage(Page):
    ...
    category = models.CharField(
        max_length=50,
        choices=[
            ('technology', '技术突破'), ('product', '产品发布'),
            ('investment', '投资融资'), ('research', '研究突破'),
            ('policy', '政策法规'), ('industry', '行业动态'),
            ('startup', '创业公司'), ('academic', '学术研究'),
        ],
        default='technology',
        verbose_name="资讯分类"
    )
...
**AI工具API (apps/api/rest/ai_tools.py)**:
...
**AI工具搜索模板 (configs/search_templates/ai_tools.json)**:
...
```json
{
  ...
  "query": {
    "function_score": {
      ...
      "functions": [
        {"exp": {"publish_time": {"origin":"now","scale":"24h","decay":0.5}}},
        {"field_value_factor": {"field":"rating","factor":2,"missing":1}},
        {"field_value_factor": {"field":"usage_count","factor":0.1,"missing":1}}
      ],
...
**Zustand状态管理 (src/store/aiPortalStore.ts)**:
...
export const useAIPortalStore = create<AIPortalState>()(
  persist(
    (set, get) => ({
      ...
      setSelectedToolCategory: (category) => set({ selectedToolCategory: category }),
      setSelectedNewsCategory: (category) => set({ selectedNewsCategory: category }),
      
      // 计算值
      getHotTools: () => get().tools.filter(tool => tool.is_hot),
      getNewsByCategory: (category) => get().news.filter(news => news.category === category),
    }),
    {
...
import { Menu, X, Brain, Zap, Newspaper, User, Search, TrendingUp, BookOpen } from 'lucide-react';
...
import { Brain, Zap, Newspaper, Star, TrendingUp, Clock, ExternalLink, ArrowRight, Search, BookOpen, Users, Globe } from 'lucide-react';
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center space-x-3">
            ...
            <span className="text-red-800 font-medium">
              {news.length > 0 && news.find(n => n.is_top)?.title || "最新AI技术动态，敬请关注"}
            </span>
          </div>
        </div>
      </section>
...
**AI资讯页面 (src/app/news/page.tsx)** 和 **AI教程页面 (src/app/tutorials/page.tsx)** 采用类似的实现模式。
...
- **Wagtail后台**: http://localhost:8000/admin/
...
- 使用Django缓存框架缓存API响应
- 优化数据库查询，使用select_related()和prefetch_related()
- 实现分页和搜索索引
- 使用OpenSearch进行全文搜索
...
本开发指南详细记录了基于IDP-CMS架构构建AI门户网站的完整过程，包括：

1. **项目初始化**：Git配置、容器环境搭建
2. **后端开发**：Wagtail模型设计、API端点开发、数据库迁移
3. **前端开发**：Next.js应用构建、TypeScript配置、组件开发
4. **整合测试**：API测试、前后端联调
5. **部署准备**：容器化配置、环境变量管理
...
- **模块化设计**：前后端分离，便于维护和扩展
- **现代化技术栈**：使用最新的Next.js 15和Wagtail 7.1
- **高性能**：支持OpenSearch全文搜索和ClickHouse分析
- **易于部署**：基于Docker的容器化部署
- **可扩展性**：支持多租户和微服务架构

通过遵循本指南，您可以快速构建一个专业的AI门户网站，并为后续的功能扩展和性能优化打下坚实的基础。
